<template>
  <el-dialog
    ...
    <el-form-item label="优惠?砝嘈蚚0->全场赠券；1->会员赠券；2->购物赠券；3->注册赠券]" prop="couponType">
      <el-input v-model="dataForm.couponType" placeholder="优惠?砝嘈蚚0->全场赠券；1->会员赠券；2->购物赠券；3->注册赠券]"></el-input>
    </el-form-item>
  ...
  export default {
    data () {
      return {
        ...
        dataRule: {
          couponType: [
            { required: true, message: '优惠?砝嘈蚚0->全场赠券；1->会员赠券；2->购物赠券；3->注册赠券]不能为空', trigger: 'blur' }
          ],
          couponImg: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/mymallcoupon/coupon/info/${this.dataForm.id}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/mymallcoupon/coupon/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
